...
void MprpcProvider::NotifyService(std::shared_ptr<google::protobuf::Service> service) {
    //服务信息， 包括服务对象和服务的方法表
...
void MprpcProvider::Run() {
    ...
    //创建tcp_server对象
    muduo::net::TcpServer tcp_server(&event_loop_,address,"MprpcProvider");

    using namespace std::placeholders;
    //设置新连接创建和断开的回调函数
    tcp_server.setConnectionCallback(std::bind(&MprpcProvider::OnConnection, this, _1));

    //设置用户读写事件的回调函数
    tcp_server.setMessageCallback(std::bind(&MprpcProvider::OnMessage, this, _1, _2, _3));
    ...
    //service_name 为永久性节点， mathod_name为临时性节点
    for (auto& sp : service_map_) {
        std::string service_path = "/" + sp.first;
        //创建永久性节点
        zk_client.Create(service_path.c_str(), nullptr, 0);
        for (auto& mp : sp.second.method_map_) {
            std::string mathod_path = service_path + "/" + mp.first;
            char mathod_path_data[128] = {0};
            sprintf(mathod_path_data,"%s:%d",ip.c_str(), port); 
            //创建临时性节点
            zk_client.Create(mathod_path.c_str(),mathod_path_data, strlen(mathod_path_data), ZOO_EPHEMERAL);
        }
    }
...
void MprpcProvider::OnMessage(const muduo::net::TcpConnectionPtr& conn,    //连接
    ...
    google::protobuf::Closure* done = 
        google::protobuf::NewCallback<MprpcProvider, const muduo::net::TcpConnectionPtr&, google::protobuf::Message*>(this, &MprpcProvider::SendRpcResponse, conn, response);
